...
export async function runSync() {
  ...
  for (const issue of issues) {
    try {
      ...
      // if (issueDetail.key !== "SE-128") {
      //   continue;
      // }
      // console.log("den roi");
      const aggregateTimeEstimateSecond =
        issueDetail?.fields?.aggregatetimeestimate || 0;
      const aggregateTimeOriginalEstimateSecond =
        issueDetail?.fields?.aggregatetimeoriginalestimate || 0;
      const aggregateTimeSpentSecond =
        issueDetail?.fields?.aggregatetimespent || 0;
