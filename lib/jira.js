...
const MINI_DEFAULT_FIELDS = ["id", "subtasks"];
...
export async function fetchUpdatedIssues(sinceIso) {
  ...
  return iterateJql({ jql, fields: MINI_DEFAULT_FIELDS });
}

export async function fetchUpdatedIssuesDetail(id) {
  const parts = [`id = ${id}`];
  if (JIRA_PROJECT_KEY) parts.push(`project = ${JIRA_PROJECT_KEY}`);
  if (JIRA_JQL_EXTRA) parts.push(`(${JIRA_JQL_EXTRA})`);
  const jql = `${parts.join(" AND ")}`;

  return iterateJql({ jql, fields: ALL_FIELDS });
}
...
export async function downloadAttachment(contentUrl) {
